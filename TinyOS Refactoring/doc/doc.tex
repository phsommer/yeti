...
When the {\it createchange()} method is called, a {\it CompositChange} is created. Then the method {\it addChange()} loops over all the identifiers in the {\it affectedIdentifiers} map 
and creates a {\it TextFileChage} to change the old name to the new name.

A special thing in NesC is that the interface file has the same name as the interface. That's why also a {\it RenameResourceChange} is added to rename the interface file
to the new interface name. In the end the whole {\it CompositChange} is returned to the LTK Framework.
...
\nomenclature{Yeti}{The TinyOS 2.x Plugin for Eclipse, nicknamed "Yeti 2", was developed by the Distributed Computing Group at ETH Zurich. The plugin aims to provide developers with all the convenience functions expected from a modern development environment.}
\renewcommand{\nomname}{Glossary}
\printnomenclature
